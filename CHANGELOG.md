...
- **BREAKING: 코어 골격 전면 재설계 — 레인(lane) 모델을 프로젝트/세션/바인딩 모델로 교체.** v0.2.x 의 `(source × backend × project)` 레인 단일 축을 프로젝트(vault + 설정)·세션(대화 1건, `active`/`hibernated`/`detached`/`archived` 상태)·바인딩(채널 주소↔세션)의 3단 모델로 재설계했다. 하위호환은 보장하지 않는다(major 변경) — 유일한 보장은 **기존 v0.2.x 데이터를 읽지도 변경하지도 않는다**는 것이다(물리적으로 분리된 설정 루트 `~/.config/adde/projects/<proj>/` 사용, 구 루트는 `~/.config/adde/<proj>/` 그대로 보존).
  ...
  - **무손실 대화 이벤트 기록 + 재생성 가능한 노트**: 모든 턴의 모든 이벤트를 `events-NNNN.jsonl`(세대 분할, 삭제 없음)에 append 하고, 턴/세션/프로젝트 노트는 그 기록의 순수 파생물로 렌더한다 — `adde vault rebuild` 로 언제든 노트를 이벤트 기록만으로 정확히 재생성할 수 있다. v0.2.x 의 회전 삭제 transcript·out 원장은 제거됐다.
  - **엔진 드라이버 레지스트리**(`EngineDriver`/`EngineCaps`/`ENGINE_REGISTRY`) — 엔진은 자신의 지원 범위(재개·권한·스트리밍·사용량·압축·첨부)를 선언하고 코어는 그 선언만 읽는다. 현재 등록은 `acp`(claude-agent-acp) 1종.
  ...
  - **엔진 진단 로그를 세션별로 기록** — 엔진 어댑터 프로세스의 진단 출력(stderr)이 세션마다 별도 로그로 남아 `adde logs <proj> <session> --engine` 으로 확인할 수 있다(5MB 세대 회전). 엔진·어댑터 계층의 기동 실패나 이상 종료 원인을 ADDE 자체 이벤트 기록과 별개로 추적할 수 있다.
  - **재개 실패가 엔진 프로세스를 남기지 않는다** — 저장된 재개 핸들로 엔진에 붙는 데 실패해 세션이 `detached` 로 확정될 때, 그 시도에서 띄운 엔진 프로세스를 함께 정리한다(재개 실패가 반복돼도 프로세스가 누적되지 않는다).
  - **입력 노트를 불필요하게 다시 쓰지 않는다** — 사용자 조작이 없으면 활성 세션의 입력 노트(`inbox.md`)를 감시 주기마다 재기록하지 않는다. 이전에는 내용이 같아도 매 주기 파일을 다시 써서 동기화 서비스가 계속 변경을 감지했고(업로드 트래픽·리비전 이력 오염), 읽기와 쓰기 사이에 작성 영역에 입력한 내용이 덮일 수 있는 창이 상시 열려 있었다.
  - **노트 저장 실패를 조용히 넘기지 않는다** — 턴이 끝난 뒤 노트 저장(투영)이 실패하면 세션에 경고를 남긴다(`adde session show <proj> <session>` 의 `warnings`). 저장 위치가 읽기전용이거나 권한이 막힌 경우 이전에는 응답만 정상적으로 전달되고 저장 실패는 어디에도 드러나지 않아 대화가 저장된 것으로 오인할 수 있었다. 경고는 저장소(vault)가 아닌 설정 루트에 기록되므로 저장소 자체가 쓰기 불가여도 남는다(대화 내용은 무손실 이벤트 기록에 온전히 남아 있고 `adde vault rebuild` 로 노트를 복원할 수 있다).
  - **`adde status` 에 `WARN` 컬럼 추가** — 세션에 기록된 경고 건수를 표에 함께 보여준다(없으면 `-`). 본문은 표에 싣지 않고 `adde session show <proj> <session>` 으로 확인한다. `--json` 은 세션 행에 `warnings` 배열이 더해지는 추가 변경이라 스키마 버전은 그대로이고 기존 소비자는 영향받지 않는다.
  - **입력 노트에 상태 존 신설** — 해소되지 않은 경고(저장 실패·재개 실패)가 있으면 팔레트 위에 `<!-- adde:status -->` 블록이 나타나 그 사실을 알린다. 세션 기록의 파생물이라 조건이 해소되면 스스로 사라지고(다음 턴이 정상 저장되거나 재개가 성공하면) 별도 정리가 필요하지 않다. 작성 경계 **위**에 두므로 경고문이 다음 지시 본문에 섞이지 않으며, 경고에 섞인 엔진 생성 텍스트는 노트에 넣기 전에 살균한다(개행·제어문자를 접어 체크박스 구조를 위조할 수 없게 한다).
  - **같은 종류의 경고가 누적되지 않는다** — 재개 실패가 반복되면 데몬 재기동마다 경고가 한 건씩 늘어 최신 사유를 읽기 어려웠다. 이제 종류별 최신 1건만 유지한다.
  - **진단 출력 문구 정정** — `adde logs <proj> <session> --engine` 이 "로그 파일이 없음"(엔진이 아직 기동하지 않음)과 "로그가 비어 있음"(엔진이 진단 출력을 남기지 않음)을 구분해 안내한다. 데몬 로그는 유휴 내림·초기화 같은 의도적 종료를 더 이상 "crash" 로 기록하지 않는다(자가 재기동 동작 자체는 종전과 같다).
  ...
  - **턴 처리 중단이 조용히 사라지지 않는다** — 턴시작 기록·선투영·엔진 투입·응답 기록·턴종료 기록 중 어느 단계에서 실패해도 세션에 `turn-failed:` 경고를 남기고(다음 턴이 성공하면 자동 해소), 이미 `처리 중`으로 표시돼 있던 턴 노트는 `오류`로 종결한다. 이전에는 데몬 로그에만 남고 사용자에게는 응답이 영원히 오지 않는 것으로만 보였다.
  - **손상되거나 적재에 실패한 지시가 조용히 사라지지 않는다** — 큐 메시지가 파싱 불가로 격리되면 `quarantined:` 경고, 마크다운 입력 노트의 지시를 큐에 넣는 데 실패하면 `enqueue-failed:` 경고를 남긴다(성공 시 자동 해소).
  - **팔레트(clear/compact/resume) 조작 실패가 경고로 남는다** — 이전에는 체크만 풀리고 실패 사유가 어디에도 남지 않았다(`palette-failed:` 경고, 성공 시 자동 해소). `clear` 가 세션 교체는 성공했지만 입력 노트 이름 변경만 실패한 경우(부분 실패)는 새 세션 쪽에 별도로 경고를 남긴다. 팔레트의 인자 없는 `resume` 체크가 아무 동작도 하지 않던 결함(파싱이 처리기 없는 종류로 매핑)도 함께 고쳐 엔진 재개로 정상 동작한다.
  - **세션 처리가 서로 격리된다** — 이전에는 폴 주기 전체가 하나의 오류 경계라 한 세션의 노트 처리 예외가 같은 주기의 다른 세션 처리(전송 적재·승인 소비 포함)를 건너뛰게 했다. 이제 세션 단위로 오류를 흡수해 한 세션의 지속적 실패가 다른 세션을 굶기지 않는다.
  - **세션 기동(TurnRunner) 실패가 경고로 남는다** — `runner-failed:` 경고, 다음 정상 처리에서 자동 해소.
  - **권한 결정 세 등급을 모두 기록한다** — 하드 차단·자동 허용(`allowlist`/`autopass`)도 채널 승인과 동일하게 이벤트 기록에 요청·결정 쌍으로 남는다(결정 경로 필드 `via`). 턴 노트의 "권한 요청·결정" 섹션에서는 자동 허용 건을 접기 요약(`자동 허용 N건`)으로 묶어 소음 없이 감사 가능성을 유지한다.
  - **권한 설정 차이 경고와 `autopass` 기동 배너를 복원한다** — 엔진의 실효 권한 모드가 ADDE 정책보다 느슨하면(예: bypassPermissions) "상황 + 조치" 2요소 형식의 경고를 세션에 남긴다(이전 구현이 존재하지 않는 스키마 필드를 읽어 이 경고가 한 번도 발화하지 않았던 결함을 함께 고쳤다). `autopass` 티어 프로젝트를 기동하면 `adde up`/`adde restart` 출력과 부팅 리포트(`--json` 포함)에 자동 허용 모드임을 알리는 안내가 실린다.
  - **`file_mode` 적용자를 복원한다** — 프로젝트 생성 시점과 데몬 기동 시점에 설정 루트의 내부 디렉터리(프로젝트 루트·세션 레코드·런타임 트리)를 conf 선언대로 잠근다(기본 `private`→0700). `shared` 는 그대로 아무 것도 하지 않는다. vault 노트 트리는 대상이 아니다.
  - **`--safe-defaults` 를 구현한다** — `adde project add --safe-defaults` 로 생성하면 내장 위험 명령 목록을 `hard_deny` 에 명시 지정분과 합집합으로 시드한다(문서가 이미 약속했던 동작이나 실제로는 미구현이었다).
  - **npm 패키지·설치 명령은 변경 없음** — `npm i -g adde-acp`, 실행 명령 `adde`.

### Security

- **보관 이관을 실행하기 직전에 보관 위치 겹침을 다시 확인한다** — 보관 위치(`vault.backup`)가 저장소(vault)·설정 루트·프로젝트 실행 경로와 겹치는지는 `adde project add`/`adde project set` 이 이미 검증한다. 다만 설정 파일을 CLI 없이 직접 편집했거나 검증 이후 저장소·설정 루트가 옮겨진 경우에는, 겹친 경로가 그대로 하루 1회 자동 이관에 쓰여 턴 노트가 저장소·설정·작업 경로 안으로 옮겨질 수 있었다. 이제 이관 실행 직전에 같은 검증을 한 번 더 수행하고, 겹치면 이관을 건너뛰고 세션 노트에 경고를 남긴다(하루 1회). 설정을 고치면 다음 점검 주기에 곧바로 정상 이관이 재개된다.
- **승인 노트에 삽입되는 엔진 생성 텍스트를 살균한다** — 권한 요청의 도구명·작업 디렉터리 등 엔진(모델)이 자유롭게 생성하는 텍스트에서 개행·제어문자를 공백으로 접고 길이를 제한한 뒤에만 마크다운 승인 노트에 렌더한다. 엔진 출력에 개행이나 마크다운 체크박스 구문이 섞여도 승인 노트의 체크박스 구조를 위조할 수 없다(렌더 직후 자가 검증도 추가되어, 정규화 후에도 이상 결정이 감지되면 파일을 쓰지 않고 fail-closed 로 거부한다). 로그·턴 노트에 재렌더되는 권한 정보에도 동일한 살균이 적용된다.
