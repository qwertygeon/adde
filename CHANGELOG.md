...
- **BREAKING: 코어 골격 전면 재설계 — 레인(lane) 모델을 프로젝트/세션/바인딩 모델로 교체.** v0.2.x 의 `(source × backend × project)` 레인 단일 축을 프로젝트(vault + 설정)·세션(대화 1건, `active`/`hibernated`/`stopped`/`detached` 상태)·바인딩(채널 주소↔세션)의 3단 모델로 재설계했다. 하위호환은 보장하지 않는다(major 변경) — 유일한 보장은 **기존 v0.2.x 데이터를 읽지도 변경하지도 않는다**는 것이다(물리적으로 분리된 설정 루트 `~/.config/adde/projects/<proj>/` 사용, 구 루트는 `~/.config/adde/<proj>/` 그대로 보존).
  ...
  - **BREAKING: 설정의 경로 값은 절대경로만 허용한다.** 프로젝트 설정의 `vault`·`vault.backup`·`cwd` 는 `~` 확장 후 절대경로여야 하고, 상대경로가 남아 있으면 설정 읽기 자체가 거부된다 — 그 프로젝트의 조회·편집 명령과 데몬 기동이 모두 실패한다(경로를 실행 위치에 따라 다르게 해석하면 삭제·이관 같은 파괴적 동작이 의도하지 않은 곳을 향할 수 있기 때문이다). **`~/...` 표기의 의미도 함께 바뀌었다**: 이전에는 `~` 가 확장되지 않아 프로세스의 작업 디렉터리 아래 리터럴 `~` 폴더를 가리켰고, 이제는 홈 디렉터리로 확장된다. 셸에서 따옴표 없이 `--vault ~/Vault` 를 준 경우는 셸이 이미 홈으로 펼쳤으므로 영향이 없고, **설정 파일을 직접 편집했거나 `--vault '~/Vault'` 처럼 따옴표로 감싸 `~` 가 그대로 저장된 프로젝트만 노트 위치가 달라진다** — 이때는 리터럴 `~` 폴더에 남은 노트를 새 위치로 옮기거나, `vault` 값을 그 폴더의 절대경로로 고쳐 쓰면 종전 노트를 그대로 계속 쓸 수 있다(ADDE 가 파일을 옮기거나 지우지는 않는다).
  ...
  - **markdown 노트 존 레이아웃 재정의** — 상주 팔레트(기록 그룹 `archive` / 세션 그룹 `compact`·`clear`·`stop`·`resume`)·`<!-- adde:compose -->` 작성 경계·`<!-- adde:records -->` 기록 존은 유지되나, 링크 대상이 별도 아카이브/out 노트가 아니라 **턴 노트**(무손실 기록의 직접 파생물)로 바뀌었다. 별도 전송 아카이브 파일은 더 이상 존재하지 않는다.
  ...
  - **세션 중지(stopped) 를 1급 상태로 신설 — 다 쓴 세션이 주기 작업에서 빠진다.** 영속 세션 상태를 **활성·유휴·중지·떨어짐** 4종으로 정리했다. 중지·떨어진 세션은 입력 노트·승인 디렉터리를 **어떤 주기로도 읽지 않으며**(폴당 파일 접근 0회) 명시적 재개로만 되살아난다. 기존 `archived`(보존 종료)는 중지로 통합됐고, 레코드에 남아 있는 `archived` 값은 **읽을 때 중지로 해석**되며 파일을 고쳐 쓰지 않는다(마이그레이션 명령 없음). `adde status --all`(집계 뷰)은 이제 `stopped`·`detached` 세션을 포함한다.
  - **자동 중지(기본 켬)** — 마지막 활동 이후 `stop_after_min`(신설 키, 기본 60분)이 지난 세션을 자동으로 중지하고 사유("무활동")를 노트에 남긴다. `idle_stop`(신설 키, 기본 `true`)으로 옵트아웃한다. 유휴 내림(`hibernate_after_min`, 기본 30분)은 종전과 같고 여전히 먼저 일어나며, 두 임계 모두 **마지막 활동 이후 경과**를 기준으로 한다 — 중지 임계가 유휴 임계보다 작거나 같으면 유휴를 건너뛰고 곧바로 중지된다. 상주 엔진 상한(`max_active_engines`) 초과로 내려가는 세션은 여전히 유휴로만 내려가며 중지되지 않는다(기존 동작 불변).
  - **중지 예약** — 진행 중인 턴·잔여 큐·미소비 전송 체크박스가 있으면 즉시 중지하지 않고 "완료 후 중지"로 예약하고 그 사실을 안내로 남긴다. 잔여 작업이 소진되면 실제로 중지하며 완료 안내를 한 번 더 남긴다. 예약은 데몬 재기동을 건너 유지되고, 유지할 수 없는 경우에는 그 사실을 안내로 표면화한다(조용한 소실 없음).
  - **`session stop`/`session resume` CLI 신설** — 노트 경로와 대칭을 이룬다(활성 세션이 하나도 없으면 노트 경로가 성립하지 않으므로 CLI 경로가 필수다). 상태 불일치(이미 중지된 세션에 중지, 대상이 아닌 세션에 재개)·재개 대상 0건·없는 식별자·형식 오류를 각각 안내하며 무동작을 성공으로 보고하지 않는다. 데몬 상주 중에도 실효한다 — 요청이 데몬이 소진하는 control 큐를 경유하고, 결과를 관측할 수 없으면 명령이 거부하며 재기동 후 재시도를 안내한다. **데몬이 꺼진 상태에서 실행해도 노트가 함께 갱신된다** — 중지 시 안내형 노트로 교체, 재개 시 정상 스켈레톤 복구, 미소비 전송 체크박스 확인(중지 예약 판단)이 데몬 경로와 동일하게 동작한다.
  - **입력 노트에 안내 존 신설** — 경고 존과 같은 작성 경계 **위**에 `<!-- adde:notices -->` 영역이 생겨 "알려야 하는데 알려지지 않던" 사건을 표면화한다(세션 중지·중지 예약·**대화 압축 성공**·세션 승계 링크·재개 결과·목록 절단·안내 프루닝 등). 항목마다 체크박스가 붙고 **체크하거나 그 줄을 직접 지우면 읽음 처리**된다. 세션 레코드의 파생 렌더라 노트 치유·데몬 재기동을 건너 유지된다. 최신 `markdown.notices_cap`(신설 키, 기본 10)건을 유지하며 **`0` 은 무제한**이고, 초과 제거 시 그 건수를 안내로 남긴다. 경고 존과 역할을 분리했다 — 경고 존은 **미해소 실패**만 담고 성공 시 자동 해소되며, 안내 존은 **읽으면 끝나는 알림**만 담는다.
  - **팔레트를 기능 그룹으로 재편 + `stop` 신설** — 기록 그룹(`archive`) / 세션 그룹(`compact`·`clear`·`stop`·`resume`)으로 묶고 그룹 머리글을 체크박스가 아닌 줄로 렌더한다(액션으로 파싱되지 않고 초안으로도 오인되지 않는다). 신설 `⏹️ stop` 은 그 세션만 중지한다. `🧹 clear` 는 이제 이전 세션을 중지하고 두 노트를 양방향으로 링크한다(새 세션에 이전 세션 링크, 중지된 세션에 승계된 새 세션 링크).
  - **BREAKING: 팔레트 `♻️ resume` 의 의미 변경 — "자기 세션의 엔진 재개" 항목이 없어졌다.** 이제 `resume` 은 **중지·떨어진 다른 세션을 재개**한다. 활성 세션 노트에서 인자 없이 체크하면 다음 주기에 안내 존이 대상 목록(최근 10건, 절단 시 절단 사실 + 전체 조회 방법)을 체크박스로 렌더하고, 하나를 체크하면 그 세션이 재개된다. `resume <식별자>` 형태는 목록 단계를 건너뛴다. 유휴 세션은 다음 지시에서 투명하게 재개되고 떨어진 세션은 다른 세션의 목록에서 다뤄지므로 기능 공백은 없다.
  - **중지·떨어짐·제거된 세션의 입력 노트를 안내형으로 1회 교체** — 팔레트와 전송 체크박스를 **전부**(기록 그룹 포함) 제거하고, (a) 이 노트가 감시되지 않는다는 사실 (b) 사유 (c) 재개 두 경로(활성 세션 팔레트의 `resume`·터미널의 `adde session resume`)를 담는다. 아무도 폴하지 않는 노트에 남은 체크박스는 영구히 소비되지 않기 때문이다. 초안 텍스트와 기록 존은 보존되며, 재개하면 정상 스켈레톤이 초안과 함께 1회 복구된다. 노트 교체가 실패하면 상태 전이와 구분되는 부분 실패로 경고에 남는다(상태는 중지인데 노트는 정상형인 상태를 침묵하지 않는다).
  - **세션 식별자를 사람이 고를 수 있는 형식으로 변경** — 신규 세션은 `YYMMDD-N`(생성 시점 로컬 날짜 + 그날 순번, 예: `260828-2`), 제목이 있으면 `YYMMDD-N-<slug>` 를 받는다. 부여 시점에 세션 레코드·vault 노트 디렉터리·이벤트 기록 디렉터리 **세 위치**와 충돌하지 않도록 순번을 진행시킨다(일반 제거로 레코드만 사라지고 남은 디렉터리에 새 세션이 얹히지 않는다). **기존 식별자는 개명하지 않으며** 두 형식이 공존한다. 목록(세션 목록·재개 목록)은 식별자 사전순이 아니라 **마지막 활동 시각** 기준으로 정렬하고 제목·마지막 활동 시각을 병기한다.
  - **BREAKING: 본문 저장소·중복 판정 원장을 세션 소유로 분리하고 교차 세션 중복 판정·링크를 폐기한다.** 첨부·임계 초과 도구 출력의 내용 주소 저장소와 중복 판정 원장이 프로젝트 공유(`.adde/blobs`·`.adde/ledger/dedup.jsonl`)에서 **세션 소유**(`.adde/sessions/<sid>/blobs`·`.adde/sessions/<sid>/dedup.jsonl`)로 내려갔다. 같은 내용을 서로 다른 두 세션에 보내면 **더 이상 중복으로 판정되지 않고 링크도 생기지 않으며** 각 세션이 본문을 온전히 보유한다(한 세션 안에서의 중복 판정은 종전대로 유지). 이에 따라 **"내용이 같으면 실제 저장은 한 번만" 이라는 기존 보장이 프로젝트 전체에서 세션 내로 축소된다** — 같은 첨부를 두 세션이 보유하면 디스크의 실체도 두 개이고 동기화 업로드도 두 번 일어난다. 대가로 얻은 것은 파괴적 경로의 안전성이다: 완전 제거가 세션 소유 디렉터리 삭제로 완결되어 "다른 세션이 아직 참조하는가" 판정이 사라지고 오삭제 위험이 구조적으로 제거된다. 재기동 후에도 그 세션 내부의 중복 판정이 유지되도록 판정 인덱스 시드를 배선했다(이전에는 시드 함수에 호출처가 없어 재기동마다 판정 이력이 무증상으로 사라졌다).
  - **기존 데이터는 이관하지 않는다(무이관)** — 배치 변경 이전에 기록된 본문 저장소·원장은 이전 위치에 **그대로 남으며** 이동·변경·삭제되지 않는다(실행할 마이그레이션 없음). 그 결과 **배치 변경 이전에 생성된 세션은 완전 제거가 이전 위치의 승격 본문을 지우지 못한다** — 이 한계는 완전 제거의 확인 문구와 결과 안내에 그대로 표기되며, 완전한 보장은 `adde factory-reset` 으로만 가능하다(이전 배치의 원장에서는 그 세션 id 를 가리키는 라인만 제거된다).
  - **BREAKING: `session rm` 재설계 — `--force` 제거.** 대화형에서 옵션 없이 실행하면 지울 대상(경로·턴 수·진행 중 턴 유무·구 배치 여부)을 먼저 보여준 뒤 **완전 제거 / 일반 제거 / 취소** 3분기를 묻는다. **완전 제거**는 그 세션 소유의 전부(설정 루트의 레코드·큐·처리중·세션 런타임·엔진 로그 + vault 의 노트·승인 노트·이벤트 기록·본문 저장소·중복 판정 원장)를 지우며 다른 세션의 파일은 하나도 건드리지 않는다. **일반 제거**는 설정 루트 쪽만 지우고 vault 를 전부 보존해 세션이 목록에서만 사라지며, 재생성 명령이 필요하지 않다(남는 입력 노트는 "제거됨" 안내형으로 1회 교체된다). **취소**는 아무것도 지우지 않고 실패 종료한다. `--purge` 는 확인 없는 완전 제거로 스크립트·비대화 전용이며, **비대화에서 옵션 없이 실행하면 아무것도 지우지 않고 사용법과 함께 거부한다**(fail-closed). 대상 부재는 "없음"으로 알리고 실패 종료하며(성공 위장 없음), 삭제 실패는 모아 경고하고 실패 종료한다. 이전 동작에서는 옵션 없는 `session rm` 이 in-memory 레코드만 지우고 "삭제됨"을 보고했고, `--purge` 는 확인 없이 즉시 삭제하면서 이벤트 디렉터리가 세션 디렉터리 밖에 있어 **대화 원본을 남겼다**. 구 `--force` 는 선언만 있고 읽히지 않았다.
  - **`adde factory-reset` 신설 — 모든 프로젝트와 세션을 지워 처음 설치 상태로 되돌린다.** 인자·플래그가 없다. **명령 전용**으로 노트·팔레트 진입점을 만들지 않으며(동기화 충돌 사본이나 오체크 하나로 설치 전체가 파괴되는 것을 막는다), **비대화 환경에서는 거부**한다(fail-closed). 실행 전에 인벤토리(프로젝트 수·프로젝트별 세션 수·vault 경로·보존 대상)를 보여준 뒤 **고정 문구 타이핑**을 요구하며 예/아니오 단축은 없다. 삭제는 **인벤토리에 실린 프로젝트 단위**로 이뤄진다 — 각 프로젝트의 설정 디렉터리와 그 프로젝트의 **vault ADDE 서브트리**를 지우고, 설정 컨테이너 자체는 비게 된 경우에만 정리한다. **vault 루트 자체와 ADDE 네임스페이스 밖 사용자 파일은 건드리지 않고**, **v0.2.x 데이터도 건드리지 않으며 결과에 보존 사실을 명시한다**. 설정을 읽을 수 없는 디렉터리(이름이 허용 문자셋 밖이거나 설정 파일이 손상된 경우)는 **일부러 남기고** 인벤토리에 "해석 불가 — 삭제 대상에서 제외" 로 열거한다(고지한 범위와 실제 삭제 범위를 일치시킨다). 각 프로젝트의 데몬을 먼저 정지하고 잔존을 확인하며, 데몬이 살아 있으면 **삭제를 하나도 하지 않는다**(상주 데몬이 지운 레코드를 되만들어 반쯤 초기화된 상태가 되는 것을 막는다). 설정이 가리키지 않는 vault 잔존물은 자동 삭제하지 않고 열거·별도 확인하며, 어떤 설정에도 언급되지 않은 vault 의 잔존물은 발견 대상이 아니라는 한계를 결과에 밝힌다. 부분 실패는 성공으로 보고하지 않고 남은 경로를 열거하며 실패 종료한다.
    - **vault 를 찾을 수 없는 프로젝트**(경로 없음·권한 거부·링크 루프 등)는 인벤토리에 경로와 사유가 함께 표시되고, 고정 문구 확인 뒤 **별도 질문**으로 "설정만 지울지" 를 따로 묻는다(기본은 아니오). vault 쪽은 어느 경우에도 손대지 않으며 — 삭제 반경을 확정할 수 없기 때문이다 — 동의하지 않으면 설정을 남겨 vault 에 남은 데이터의 위치 단서를 보존하고 그 사실을 결과에 밝힌다.
    - **삭제 직전 실경로 재확인** — 지우려는 경로의 실제 위치가 그 vault 의 ADDE 폴더 안이 아니면(중간 경로가 심볼릭 링크여서 vault 밖이나 vault 안 다른 폴더를 가리키는 경우 포함) 그 프로젝트의 **vault 삭제와 설정 삭제를 모두 보류**하고 사유와 함께 "링크를 정리한 뒤 다시 실행" 을 안내하며 실패 종료한다(설정이 vault 위치를 알려주는 유일한 근거라 설정만 지우면 남은 데이터를 다시 찾을 수 없다). vault 잔존물 삭제에도 같은 확인이 적용된다.
    - **알려진 한계**: 확인과 삭제 사이에 경로가 링크로 교체되는 경합은 막지 못한다(Node 파일 API 에 "검사한 그 대상" 을 원자적으로 지우는 수단이 없다). 동기화 클라이언트가 심볼릭 링크를 어떻게 재현하는지는 확인되지 않았으므로, 동기화되는 vault 에 ADDE 폴더 밖을 가리키는 링크를 두지 않는 것을 권한다. 확인 화면에 표시되는 외부 유래 문자열(설정의 vault 값·디렉터리 이름)은 제어문자만 접어 표시하며, 표시 폭을 바꾸는 서식·양방향 문자는 대상이 아니다(표시만 해당하고 삭제 반경은 영향받지 않는다).
  - **`session attach`/`detach` 의 의도를 문서에 명시** — 이 둘은 **TUI 와 교대로 쓰기 위한 소유권 배턴**이며 세션 중지와는 다른 개념이다(구현은 본 릴리스 범위 밖이며 문서에 미구현으로 표기한다).
  - **명령 문서의 오서술 2건 정정** — "확인 옵션 없는 `session rm` 이 보존 종료 상태를 만든다"(그 경로는 실제로 무동작이었다), "완전 제거 옵션은 확인 절차가 필요하다"(실제로는 확인 없이 즉시 삭제했다) 두 서술을 재설계된 실제 동작으로 교체했다.
  - **알려진 한계**: 재개 선택 목록을 **옵션이 나타난 직후 수 초 안에** 지워 취소하면 그 **첫 시도가 반영되지 않을 수 있고** 옵션이 되살아난다 — 이때 "취소가 아직 반영되지 않았다" 는 안내가 노출되며, 한 번 더 지우면 취소가 성립한다(옵션 줄을 일부만 지우면 실수 편집으로 보고 목록을 다시 그린다).
- **데몬 라이브니스 기록·주기 갱신·정상 종료 시 제거를 복원한다** — `adde up` 이 기동 시 라이브니스 기록(프로세스 식별자·기동 시각·스키마 버전)을 남기고 상주하는 동안 60초마다 갱신하며(`ADDE_HEARTBEAT_INTERVAL_MS` 환경변수로 조정 가능, 양수 정수만 유효, 기본 60000) 정상 종료 시 제거한다(기존에는 기록자·주기 갱신·제거 호출이 모두 유실되어 있어 상주 표시가 실제 신호가 아니라 항상 고정값이었다). 세션의 `PRESENT` 표시도 이 실제 라이브니스에서 파생된다.
- **[behavior-change] `status` 가 데몬 상태를 5종으로 구분해 표면화한다** — 표 아래 프로젝트별 상태 줄에 **상주 중 · 응답 없음(180초 이상 무갱신) · 비정상 종료 · 미기동 · 판정 불가**(라이브니스 기록이 손상돼 판독 불가) 중 하나가 표시된다. 응답 없음·비정상 종료·판정 불가와 크래시루프 자가정지 기록에는 다음에 실행할 명령을 담은 조치 안내가 함께 출력된다.
- **[behavior-change] `status` 종료코드가 등록된 전체 프로젝트 기준으로 판정된다** — 프로젝트를 지정하지 않은 집계 조회도 이제 **표시 필터(`--all`) 적용 전 전체 집합**을 기준으로 판정해, 죽은(`detached`) 세션 1건 이상 · 크래시루프 자가정지 기록 존재 · 데몬이 응답 없음/비정상 종료 · 라이브니스 또는 자가정지 기록이 판독 불가하여 상태를 판정할 수 없는 경우 중 하나라도 있으면 exit 1 을 반환한다(기존: 표시 필터를 거친 뒤에만 판정해 표에서 제외된 항목은 실패가 있어도 항상 exit 0 으로 끝났다). 데몬이 단순히 미기동인 것만으로는 실패로 보지 않는다.
- **[behavior-change] `status --json` 스키마 확장(추가만, 스키마 버전 `v` 불변)** — `sessions` 가 표시 필터(`--all`) 적용 전 전체 집합을 담고, `daemon`(프로젝트별 5종 라이브니스)·`haltUnreadable`(자가정지 기록 판독 불가 사유 — 단일 뷰는 `string | null`, 집계 뷰는 판독 불가한 프로젝트만 담는 맵) 필드가 추가된다. 기존 `sessions`·`halt` 필드는 형태·의미 모두 그대로 보존된다.
- **기동 창의 종료 요청이 정상 종료로 처리된다** — 종료 시그널 핸들러를 세션 축 조립보다 먼저 설치해, 조립이 끝나기 전에 도착한 종료 요청도 이제 그 시점까지 확보된 자원만 정리하는 정상 종료 경로로 처리하고 라이브니스 기록을 잔존시키지 않는다(기존: 조립 미완 구간에 도착한 종료 요청이 OS 기본 처리(즉시 종료)로 프로세스를 끝내 정리·상태 갱신이 모두 건너뛰어졌다). **알려진 한계**: 프로세스 시작 극초반(대략 1밀리초 이내, 데몬 코드 진입 이전 구간)에 도착하는 신호는 이 스택에서 커버할 수 없다.
- **`adde doctor` 텍스트 모드에 요약 줄이 신설된다** — 마지막 줄에 등급(`PASS`/`WARN`/`FAIL`/`INFO`)별 건수를 출력한다(기존 사용법 설명은 요약 줄이 있다고 서술했으나 실제로는 출력되지 않았다).
- **[behavior-change] `adde logs` 의 줄 수 인자가 유효하지 않으면 경고와 함께 기본값으로 대체된다** — 양의 정수가 아닌 값(예: `abc`, `0`, `-1`)을 주면 이제 경고를 stderr 에 출력한 뒤 기본값(50)을 적용한다(기존: 조용히 기본값으로 치환해 잘못된 입력을 알 수 없었다).
- **사용자 대면 문구의 구 어휘("레인")를 데몬·세션 어휘로 갱신한다(한국어·영어 동시)** — 데몬 종료 안내("레인 종료 중..." → "세션 종료 중..."), 세션 기동 실패 안내(플레이스홀더가 실제로는 세션 식별자를 담고 있었다)를 포함한다. 켜지 않은 상태의 표기는 세션 상태 `stopped`(중지)와 구분되도록 "미기동"(`not started`)으로 통일한다.
...
- **안내 존에도 같은 방어를 적용한다** — 신설 안내 존의 문구·선택지 라벨·절단 안내까지 렌더 시점에 일괄 살균하므로(이전 버전에 저장돼 있던 안내 항목도 화면에 나오는 순간 함께 덮인다) 라벨 안의 개행으로 위조 체크박스를 만들어낼 수 없다. 안내 존 안의 줄은 **존 경계 위치로** 판별해 팔레트·전송 동작으로 해석하지 않으며, 재개 선택은 그 안내가 실제로 발행한 선택지 값만 받아들인다 — 노트에 손으로 적어 넣은 값이나 위조된 줄은 "선택 없음" 으로 취급되고 목록이 다시 렌더된다.
- **데몬에 들어오는 제어 요청을 스키마로 검증한다** — 프로토콜 버전·요청 종류·필수 필드를 확인하고 미인식 요청은 처리 경로에 도달하기 전에 거부하며 사유를 결과로 남긴다(요청을 보낸 명령이 무응답 타임아웃 대신 사유를 즉시 관측한다). 이전에는 알 수 없는 요청 종류가 처리 단계의 폴백을 타고 가장 파괴적인 동작으로 향할 수 있었다.
- **원자적 파일 쓰기의 임시 파일 이름을 예측 불가능하게 만들고 배타 생성으로 연다** — 같은 호스트의 다른 로컬 사용자가 임시 경로를 미리 심볼릭 링크로 선점해 쓰기를 엉뚱한 대상으로 유도하는 경로를 막는다. ADDE 가 만드는 비공개 디렉터리(설정 루트 하위·이벤트 기록·본문 저장소)는 생성 시 소유자 전용 권한으로 잠근다 — 프롬프트 본문이 들어가는 큐·기록이 다중 사용자 호스트에서 다른 로컬 사용자에게 열리지 않게 한다.
- **`factory-reset` 확인 화면과 결과 출력의 외부 유래 문자열을 살균한다** — 설정의 vault 값·디렉터리 이름에 섞인 제어문자(실개행·단독 CR·터미널 제어 시퀀스)를 표시 직전에 접어, "삭제할 대상이 없습니다" 같은 위조 줄을 끼워 넣어 사용자가 삭제 반경을 오판하게 만드는 경로를 막는다. 살균은 표시에만 적용되고 실제 삭제 경로는 원문을 쓴다(화면과 다른 경로를 지우지 않기 위해서다).
