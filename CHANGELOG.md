...
- **`adde lane set` 로 권한 목록(allowlist·denylist·hard_deny)을 증분 편집** — `--add-allow`/`--rm-allow`(+ `--add-deny`/`--rm-deny`·`--add-hard-deny`/`--rm-hard-deny`)로 항목 하나를 목록 전체 재입력 없이 추가/제거한다. 각 플래그는 현재 목록에 합집합(add)하거나 차집합(rm)하며, 없는 항목 제거는 무시(안내 표시)한다. 전체 치환 플래그(`--allowlist` 등)와 같은 명령에서 함께 쓰거나 같은 항목을 add·rm 동시 지정하면 거부된다. 보안 목록이라 전체 교체 시 항목 누락 위험이 있던 것을 해소한다 — 특히 denylist 가 아직 내장 기본값인 autopass 레인에서 증분 편집 시 기본 항목을 먼저 시드해 `--add-deny`/`--rm-deny` 한 번이 기본 보호를 조용히 없애지 않는다. 셸 자동완성에도 노출된다(`adde completion` 재생성 후).
...
- **`lane set`/`lane show` 점표기 키 셸 자동완성** — `adde lane set <proj> <lane> <TAB>` 에서 편집 가능 키 이름(명명 플래그가 없는 `markdown.retention_days`·`markdown.archive` 등 점표기 전용 그룹 포함)이 옵션 플래그와 함께 완성되고, `lane show` 의 `[key]` 자리도 같은 키 목록으로 완성된다. 키 목록은 레인 키 스키마에서 파생되므로 새 키가 자동 반영된다. `lane set` 의 프로젝트/레인 이름 동적 완성도 함께 배선됐다(전에는 `lane set` 하위에 자동완성이 없었다). 기존 설치 스크립트에는 소급 적용되지 않으므로 `adde completion <shell>` 로 재생성해 다시 설치하면 반영된다.

### Changed

- **BREAKING(동작 변경): markdown 레인의 전송 즉시 본문 아카이브가 기본 켜짐으로 바뀐다** — 인박스 3존 레이아웃이 기본 켜짐이라, `markdown.archive` 를 지정하지 않은 기존 레인도 이제 전송 시점에 메시지 본문이 아카이브 디렉터리(기본 `sent-archive/`)로 자동 이관되고 인박스엔 압축 `✅ sent [[...]]` 마커만 남는다(전에는 `markdown.archive` 를 지정해야만 자동 아카이브가 켜졌다). `markdown.archive` 는 이제 "아카이브 활성화" 가 아니라 **아카이브 위치 오버라이드** 의미만 갖는다. 이전 동작을 유지하려면 `markdown.layout=off` 를 지정한다(레거시 인박스·묶음 off 상태는 완전히 하위호환).
...
- **Telegram 비텍스트 메시지에 안내 회신 + 인라인 버튼 라벨 현지화** — 허용된 발신자가 사진·스티커·음성 등 텍스트 없는 메시지를 보내면 조용히 버려지던 것을, 이제 해당 메시지에 "텍스트 메시지만 지원한다"는 안내를 회신한다(미허가 발신자는 종전대로 무시 — fail-closed). 권한 승인 인라인 버튼 라벨(`Allow`/`Deny`)도 하드코딩 영문에서 레인 로케일 문구(`허용`/`거부`)로 현지화했다(콜백 동작·데이터는 불변).
- **응답 노트 상단의 원시 UUID 역참조 줄(`> ↩ …`) 제거** — markdown 레인에서 이 값은 응답 노트 자기 자신의 id(파일명·inbox 위키링크와 동일)라 정보 없이 중복 표기만 됐다. 질문 발췌(`> ❓`)·시각 메타(`> 🕒`)는 그대로 유지된다.
- **`lane set` 명명 플래그 경로도 입력 시점에 셸 이스케이프 정규화** — `--cwd`/`--root` 등 명명 플래그로 준 경로가 점표기 편집과 동일하게 저장 전에 정규화된다. 전에는 conf 재파싱 시점에만 정규화돼 동작은 같았지만, `lane set` 직후 출력·`--json` 에 백슬래시가 섞인 원시 값이 그대로 보였다.
- **`adde lane set` 이 권한 티어와 맞지 않는 목록 설정을 거부한다** — `perm_tier=acp` 에서는 `allowlist` 만, `perm_tier=autopass` 에서는 `denylist` 만 게이트에 영향을 준다(반대쪽 목록은 no-op). 이제 유효 티어(같은 명령에서 `perm_tier` 를 바꾸면 그 값)에서 효과 없는 목록을 명시 지정하면 조용히 무시하지 않고 사유·해결법(예: "denylist 를 쓰려면 같은 명령에서 perm_tier=autopass 도 함께 지정")과 함께 거부한다. 위저드는 종전대로 두 목록을 모두 묻는다(같은 실행에서 `perm_tier` 를 바꿀 수 있으므로).

### Fixed

- **자가 재기동을 포기(`status:error`)한 레인을 채널에서 수동 복구(`/clear`·`/resume`)해도 `adde status` 가 계속 `error` 로 표시되던 문제** — 반복 크래시로 자가 재기동이 상한을 넘겨 포기하면 `runtime.json` 에 `error` 가 기록되는데, 사용자가 `/clear`·`/resume` 로 새 세션을 성공적으로 띄워도 그 값을 되돌리는 경로가 없어 레인이 멀쩡히 살아 있는데도 `error` 로 고착됐다(약 3분 뒤엔 `stale` 로 넘어가고, 이후 크래시에 자가 재기동도 재무장되지 않았다). 이제 수동 복구 성공 시 자가 재기동 감시자가 정상 상태로 되돌아가며(재무장 + 하트비트 재개) `runtime.json` 을 `running` 으로 되쓴다 — 자동 회복이 쓰는 것과 동일한 경로를 재사용한다(별도 쓰기 경로를 늘리지 않음).
