...
- `adde up` 이 **이미 기동 중**인 데몬을 만났을 때 실패(error/dead) 레인을 표기하지 않고 항상 종료코드 0 을 내던 문제 — 신규 기동 경로와 동일하게 실패 레인을 stderr 로 표면화하고 종료코드 1 을 반환한다.
- `adde up` 이 **데몬 부팅 자체 실패**(runtime.json 을 아무도 남기지 못함)를 "기동 중(pending)"·종료코드 0 으로 보고해 하드 실패가 성공처럼 보이던 문제 — 대기 상한까지 어떤 레인도 기동/실패를 확정하지 못하면 데몬 로그 확인 안내와 함께 종료코드 1 을 반환한다. 대기 상한은 `ADDE_UP_POLL_MS`(ms) 로 조정 가능(느린 머신).
- `adde lane rm --purge` 가 `error` 상태(기동 실패 잔존, 데몬이 살아있을 수 있음) 레인의 state/큐/토큰을 `--force` 없이 삭제할 수 있던 문제 — `error` 도 활성 레인 가드에 포함해 `--force` 를 요구한다(`proj rm` 과 가드 표면 일치).
