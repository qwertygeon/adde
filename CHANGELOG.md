...
- autopass 권한 게이트의 재귀 rm 가드 견고화 — 기존엔 `rm -rf <루트/홈/닷>` 리터럴 형태만 채널 승인으로 폴백했으나, 이제 재귀 플래그 형태를 불문하고(`rm -r`·`rm -R`·`rm -fr`·`rm -rfv`·`rm --recursive`·번들 플래그) 동일 대상 스코프(`/`·`~`·`.`)를 포섭한다. `-f` 없는 `rm -r` 등이 가드를 우회하던 공백을 차단(상대경로 대상은 종전대로 자율 허용 — 스코프 불변, `safe_defaults` 하드거부에도 동일 적용).
