...
adde status [<proj>] [--all] [--json]  # session status (all projects if <proj> omitted, --all includes archived)
...
adde session <new|ls|show|clear|rm>    # manage sessions (one per conversation)
...
- [x] v2 core: project/session/binding model, lossless record store, `EngineDriver`/`Surface` registries, markdown channel (3-zone note layout), auto-resume/hibernate, self-recovery
