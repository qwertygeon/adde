...
- **`--json`**: 기동 결과를 사람용 요약 대신 JSON 으로 출력합니다 — 내부적으로 기록되는 것과 동일한 `BootReport` 형태(`{v, bootId, bootedAt, lanes: [{lane, status, error?}], running}`, 신규 필드 없음)이며 텍스트 요약/힌트는 억제됩니다. 대기 상한 내에 대응 부팅 리포트가 나타나지 않으면 JSON 출력은 리터럴 `null` 입니다(여전히 비정상 종료). 이미 기동 중/등록 잔존 조기반환 경로는 억제된 텍스트 대신 최소 객체 `{ proj, alreadyUp: true, running }` 를 출력합니다.
...
- **`--json`**: 사람용 확인 문구 대신 `{ "proj": "<proj>", "stopped": true }` 를 출력합니다. 오류 발생 시에도 오류는 stderr 로 나가고 stdout 은 비워둡니다(exit 1).
...
- **`--json`**: 점검 목록을 사람용 심볼 목록 대신 JSON 배열로 출력합니다(각 항목의 `name`/`level`/`detail`, `WARN`/`FAIL` 시 `hint` 포함)하고, 요약 줄·업데이트 안내는 억제합니다(기계가독 출력만). 종료 코드 의미는 불변(`FAIL` 존재 → 1, 아니면 0 — 텍스트 모드와 동일). `--json` 없이 호출하면 기존과 완전히 동일합니다(additive). 점검 목록 자체(텍스트 심볼 또는 `--json` 배열, 요약 줄·힌트 포함)는 두 모드 모두 stdout 을 유지하므로 `adde doctor > report.txt` 캡처가 그대로 보존됩니다.
...
- **`--json`**(스냅샷 전용): 사람용 스냅샷·"부재" 안내 대신 `{ "proj", "lane", "path", "exists", "lines" }` 를 출력합니다(`--daemon` 은 `lane` 필드 없음) — 파일 부재는 오류가 아니라 `{ exists: false, lines: [] }` 로 표현됩니다. `--follow`/`-f` 와 `--json` 을 함께 주면 `--json` 이 우선합니다 — JSON 스냅샷만 출력되고 라이브 tail 루프에 진입하지 않습니다.
...
**`--json`**: `lane ls --json` 은 사람용 목록 대신 레인 이름 JSON 배열을, `lane show --json` 은 사람용 덤프 대신 `{ "lane", "confPath", "conf" }`(`conf` 는 파싱된 레인 conf 객체 — 토큰 필드 없음)를 출력합니다.

```bash
adde lane ls myproj --json              # 기계 판독 레인 이름 배열
