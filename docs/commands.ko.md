...
**기계 판독 출력(`--json`)**: `--json` 을 지원하는 명령은 단일 기계 판독 JSON 문서를 stdout 으로 출력합니다. 모든 `--json` 출력은 최상위에 정수 **스키마 버전 필드 `v`**(현재 `1`)를 담아 소비자가 향후 구조 변경을 감지할 수 있게 합니다(스키마는 조용히 깨지는 대신 버전 필드로 진화합니다). 유일한 예외는 `up`/`restart` 부팅 타임아웃으로, 리터럴 `null` 을 출력합니다(소비자는 null 여부를 확인). 명령별 형태는 아래 각 명령에 문서화되어 있습니다.
...
3. 셸 자동완성 설정을 제안합니다(옵트인): 셸(bash/zsh)을 감지해 활성화에 필요한 `adde completion <shell> > …` 명령을 그대로 출력합니다(아래 `completion` 참조). 셸 설정 파일을 대신 편집하지 않고 명령만 안내합니다.
4. 프로젝트·레인 이름을 묻습니다(검증: 영문·숫자·`_`·`-` 만 허용).
5. 레인 필드를 대화형으로 입력받습니다(대화형 `lane add` 와 동일한 필드). telegram 레인은 봇 토큰을 **마지막에 가려진 입력**(키 입력이 화면에 에코되지 않음)으로 받아 `.env`(0600)에 기록합니다. 비워 두면 나중으로 미루며(`--token-stdin` 또는 `.env` 직접 편집으로 설정), 그 경우 완료 출력이 토큰 저장 대신 안내가 됩니다.
6. 레인을 생성합니다.
7. 토큰 기록(또는 토큰 저장 안내) 힌트와 `adde up` 기동 힌트를 출력합니다.
...
짧은 별칭(ad, add)을 adde 명령 옆에 설치할까요? (Y/n): y
...
- **`--json`**: 기동 결과를 사람용 요약 대신 JSON 으로 출력합니다 — 내부적으로 기록되는 것과 동일한 `BootReport` 형태(`{v, bootId, bootedAt, lanes: [{lane, status, error?}], running}`, 신규 필드 없음)이며 텍스트 요약/힌트는 억제됩니다. 대기 상한 내에 대응 부팅 리포트가 나타나지 않으면 JSON 출력은 리터럴 `null` 입니다(여전히 비정상 종료). 이미 기동 중/등록 잔존 조기반환 경로는 억제된 텍스트 대신 최소 객체 `{ v: 1, proj, alreadyUp: true, running }` 를 출력합니다.
...
```bash
adde up myproj --json   # 기계 판독 기동 결과(모니터링/스크립트)
```
...
- **`--json`**: `up --json` 과 동일한 부팅 리포트 JSON(위 참조) — `up`·`restart` 가 동일한 결과 렌더링 코드 경로를 공유합니다.
...
- `dead`·`stale`·`error` 레인이 있으면 조치 안내를 **stderr 로** 덧붙입니다(`[behavior-change]` — 기존에는 stdout; `SEEN` = 마지막 하트비트 경과; `error` 는 기동 실패 사유와 `adde logs <proj> --daemon`/`--engine` 포인터).
- 하트비트: `adde up` 이 주기적으로 상태 파일 mtime 을 갱신합니다. pid 가 살아있어도 갱신이 임계 시간 멈추면 `stale`(행) 로 판정합니다.
- **크래시루프 자가 정지(`halt`)**: 데몬이 짧은-수명 크래시 반복으로 자가 정지했으면([크래시 안전·로그 회전](troubleshooting.ko.md#크래시-안전--로그-회전) 참조) `status` 가 해당 프로젝트에 경고를 **stderr 로** 출력하고(`[behavior-change]` — 기존에는 stdout) **종료 코드도 1** 을 반환합니다(기존 `dead`/`stale`/`error` → exit 1 규칙은 그대로 유지됩니다). 인자 없는 집계 뷰는 화면에 표시되는 레인 목록이 아니라 대상 프로젝트 전체를 기준으로 halt 를 판정합니다 — 어떤 프로젝트의 모든 레인이 `stopped` 라 기본 집계 표에서 제외되더라도, 그 프로젝트의 halt 경고와 exit 1 은 그대로 반영됩니다.
- **[BREAKING] `--json`**: 최상위 JSON 출력이 배열이 아니라 **`{ "v": 1, "lanes": [...], "halt": ... }` 객체**입니다(기존에는 `adde status --json` 이 레인 객체 배열을 최상위로 출력했습니다). `v` 는 스키마 버전입니다(전역 옵션 참조). `lanes` 는 기존과 동일한 레인별 객체를 담습니다(`lastSeenAt` 포함, 집계 시 `proj` 부기). `halt` 는 크래시루프 자가정지 상태를 담습니다 — 단일 `<proj>` 뷰는 `HaltRecord | null`, 인자 없는 집계 뷰는 `{ "<proj>": HaltRecord | null, ... }`. **마이그레이션**: 기존 최상위 배열 참조를 `.lanes` 로 바꾸세요 — 예: `adde status --json | jq '.[]'` → `jq '.lanes[]'`. 텍스트(비-JSON) 출력은 변경 없습니다. 텍스트 표와 `--json` 본문은 모두 stdout 을 유지합니다 — 위 경고/안내만 stderr 로 이동했습니다.
- **업데이트 안내**: npm 에 새 버전이 있으면 안내 한 줄(`npm i -g adde-acp@latest` … 후 `adde restart`)을 **stderr 로** 덧붙입니다(`[behavior-change]` — 기존에는 stdout). 24시간 캐시(설정 base 하위)를 쓰며, 대화형 터미널(TTY)에서만 네트워크를 조회하고, `ADDE_NO_UPDATE_CHECK` 환경변수로 끌 수 있습니다.
- 읽기 전용(부수효과 없음).

```bash
adde status myproj          # 한 프로젝트의 레인별 표(정지 포함)
adde status --all           # 전 프로젝트, 정지 레인 포함
adde status myproj --json   # 기계 판독 {v, lanes, halt} 객체(모니터링/스크립트)
```
...
- **`--json`**(스냅샷 전용): 사람용 스냅샷·"부재" 안내 대신 `{ "v": 1, "proj", "lane", "path", "exists", "lines" }` 를 출력합니다(`--daemon` 은 `lane` 필드 없음) — 파일 부재는 오류가 아니라 `{ exists: false, lines: [] }` 로 표현됩니다. `--follow`/`-f` 와 `--json` 을 함께 주면 `--json` 이 우선합니다 — JSON 스냅샷만 출력되고 라이브 tail 루프에 진입하지 않습니다.
...
adde logs myproj tg-claude --json         # 기계 판독 스냅샷(스크립트)
```
...
- **[BREAKING] `--json`**: 최상위 JSON 출력이 배열이 아니라 **`{ "v": 1, "sessions": [...] }` 객체**입니다(기존에는 `adde sessions --json` 이 세션 항목 배열을 최상위로 출력했습니다). `v` 는 스키마 버전이며(전역 옵션 참조), `sessions` 는 기존과 동일한 항목을 담습니다 — 각 항목은 `id`/`label`/`createdAt`/`lastActivityAt`/`current`(현재 세션이면 `true`). 빈 장부는 `{ "v": 1, "sessions": [] }` 를 출력합니다(기존 `[]`; exit 0). **마이그레이션**: 기존 최상위 배열 참조를 `.sessions` 로 바꾸세요 — 예: `adde sessions --json | jq '.[]'` → `jq '.sessions[]'`. `--json` 없이 호출하면 출력·종료 코드가 기존과 동일합니다.
...
adde lane set <proj> <lane> [<key> <value> …] [--unset <key> …]  # 기존 conf 제자리 편집 (TTY 에서 인자 없이: 대화형 마법사)
adde lane ls <proj> [--json]                                     # 목록
adde lane show <proj> <lane> [key] [--json] [--defaults]         # conf 출력 (또는 단일 키 메타)
...
**`--json`**: `lane show --json` 은 사람용 덤프 대신 `{ "v": 1, "lane", "confPath", "conf" }`(`conf` 는 파싱된 레인 conf 객체 — 토큰 필드 없음)를 출력합니다. `v` 는 스키마 버전입니다(전역 옵션 참조). `key` 인자를 붙이면(`lane show <proj> <lane> <key> --json`) 대신 그 단일 키의 메타 `{ "key", "value", "default", "explicit", "editable", "identity" }` 를 출력합니다(`explicit` = 기본값을 상속하지 않고 conf 에 명시 설정된 키). `lane show <proj> <lane> --defaults --json` 은 `{ "v": 1, "defaults": [{ "key", "default" }, …] }` 를 출력합니다.

- **[BREAKING] `lane ls --json`**: 최상위 JSON 출력이 배열이 아니라 **`{ "v": 1, "lanes": [...] }` 객체**입니다(기존에는 `adde lane ls --json` 이 레인 이름 문자열 배열을 최상위로 출력했습니다). `lanes` 는 기존과 동일한 레인 이름 문자열 배열을 담습니다. **마이그레이션**: 기존 최상위 배열 참조를 `.lanes` 로 바꾸세요 — 예: `adde lane ls --json | jq '.[]'` → `jq '.lanes[]'`.

```bash
adde lane ls myproj --json              # 기계 판독 { v, lanes } 객체
adde lane show myproj tg-claude --json  # 기계 판독 conf(스크립트/모니터링)
```
...
```bash
adde lane set <proj> <lane> <key> <value> …       # 위치 점표기 편집
adde lane set <proj> <lane> --<field> <value> …   # 명명 플래그(동일 표면)
adde lane set <proj> <lane> --unset <key> …       # 키 제거(기본값 복원)
adde lane set <proj> <lane>                        # TTY 에서 인자 없이: 대화형 마법사
```

기존 레인을 삭제·재생성하지 않고 conf 를 편집합니다(state/queue/토큰 유실 없음). 지정하지 않은 필드는 기존 값을 유지합니다. 편집 지정 방식은 세 가지입니다 — 위치 점표기 키, 아래 명명 플래그, 그리고(TTY 에서 인자 없이) 대화형 마법사 — 한 명령에서 함께 쓸 수 있습니다.
...
**인자 없는 대화형 마법사(TTY)**: `adde lane set <proj> <lane>` 를 편집 인자 없이 실행하면 대화형 편집기가 뜹니다. 레인 소스의 편집 가능 필드마다 현재값이 괄호로 프리필되어 표시되며, **빈 입력은 현재값을 유지**하고, enum 은 번호로 선택, 경로 필드는 Tab 완성을 지원합니다. 마지막에 변경 요약(`key: 이전 → 이후` diff)을 출력하고 기록 전에 확인을 묻습니다 — 거절하거나 변경이 없으면 아무 것도 기록하지 않습니다. (비TTY 에서 인자 없는 `lane set` 은 마법사가 아니라 오류입니다.)
...
adde lane set myproj md-claude markdown.retention_days 5 markdown.sync_provider icloud   # 점표기 배치
adde lane set myproj md-claude --unset markdown.backup                                   # 기본값으로 되돌림
adde restart myproj
```

### lane show — 레인 conf 출력 (또는 단일 키 메타)

```bash
adde lane show <proj> <lane>                  # conf 전체(사람용 덤프)
adde lane show <proj> <lane> <key>            # 단일 키: 값·기본값·메타
adde lane show <proj> <lane> --defaults       # 편집 가능 키와 내장 기본값
```

`key` 없이 실행하면 `lane show` 는 파싱된 conf 를 출력합니다(`--json` 이면 위 **`--json`** 의 `{ v, lane, confPath, conf }` 객체). `key` 인자를 붙이면 그 키의 현재값, 내장 기본값, `explicit`(기본값 상속이 아니라 conf 에 명시 설정됐는지), `editable`/`identity` 여부를 출력합니다. 미지 키는 근접 이름 "did you mean…" 제안과 함께 거부됩니다. `--defaults`(키 없이)는 편집 가능한 모든 키를 그 기본값과 함께 나열합니다. 둘 다 `--json` 으로 기계 판독 출력을 받을 수 있습니다.

```bash
adde lane show myproj md-claude markdown.retention_days   # 예: value=5 default=2 explicit=true editable=true identity=false
adde lane show myproj md-claude perm_tier --json          # 단일 키 메타를 JSON 으로
```
...
- **`proj ls`** — 등록된 프로젝트(설정 base 아래 `lanes.d/` 를 가진 디렉터리)마다 한 행으로 레인 수·실행 수를 출력. **[BREAKING] `--json`**: 최상위 JSON 출력이 배열이 아니라 **`{ "v": 1, "projects": [...] }` 객체**입니다(기존에는 `adde proj ls --json` 이 행 배열을 최상위로 출력했습니다). `v` 는 스키마 버전이며(전역 옵션 참조), `projects` 는 기존과 동일한 행 객체(`proj`/`lanes`/`running`)를 담습니다. **마이그레이션**: 기존 최상위 배열 참조를 `.projects` 로 바꾸세요 — 예: `adde proj ls --json | jq '.[]'` → `jq '.projects[]'`.
...
- 그 명령(또는 하위 명령)에 선언되지 않은 **미지원 플래그**는 오류 메시지 + 해당 명령의 사용법(명령이 식별되지 않았으면 전체 사용법)을 stderr 에 출력하고 종료 코드 **2** 를 반환합니다 — 예: `adde doctor --nonsense`(`[behavior-change]` — 기존에는 종료 코드 1, 그 이전에는 조용히 무시하고 정상 진행 — [종료 코드](#종료-코드) 참조).
- **필수 인자 누락**(예: `<proj>` 없는 `adde up`, `<lane>` 없는 `adde lane show <proj>`)은 해당 명령의 사용법을 stderr 에 출력하고 종료 코드 **2** 를 반환합니다(`[behavior-change]` — 기존에는 종료 코드 1) — 미지원 플래그와 같은 코드입니다("호출 자체가 잘못됨"이라는 같은 범주이기 때문).

## 종료 코드

세 가지 종료 코드: **0** 성공(`--help`/`--version` 포함), **2** 호출 자체가 잘못됨 — 미지원 플래그, 값 누락/오류 플래그, 필수 인자 누락(사용법을 stderr 에 출력), **1** 그 외 성공이 아닌 모든 경우 — 운영 실패, 미지원 명령/하위명령, 값 검증 실패(`[behavior-change]` — usage/파싱 오류는 예전엔 나머지와 함께 1을 반환했습니다 — [도움말·오타 힌트](#도움말오타-힌트) 참조).

| 명령         | 0                                | 1                                                                                                  | 2                                         |
| ------------ | -------------------------------- | -------------------------------------------------------------------------------------------------- | ----------------------------------------- |
| `up`         | 데몬 등록 성공                   | launchd 등록 실패·이미 기동 중인 프로젝트에 비정상 레인 존재                                       | `<proj>` 누락·미지원 플래그/값 오류       |
| `down`       | 데몬 종료 성공(이미 없어도 0)    | 오류 발생                                                                                          | `<proj>` 누락·미지원 플래그/값 오류       |
| `restart`    | down+up 성공 + 전 레인 기동 성공 | down/up 실패, 또는 레인 1개 이상 기동 실패                                                         | `<proj>` 누락·미지원 플래그/값 오류       |
| `status`     | 모두 정상                        | `dead`(크래시)·`stale`(행)·`error` 레인 존재, **또는 프로젝트가 크래시루프 자가정지(`halt`) 상태** | 미지원 플래그/값 오류                     |
| `doctor`     | FAIL 없음(`--json` 도 동일)      | FAIL 항목 존재(`--json` 도 동일)                                                                   | 미지원 플래그/값 오류                     |
| `logs`       | 읽기 성공(파일 없어도 안내 후 0) | 경로 검증 오류                                                                                     | proj/lane 인자 누락·미지원 플래그/값 오류 |
| `sessions`   | 읽기 성공(빈 장부는 `[]`)        | 경로 검증 오류                                                                                     | proj/lane 인자 누락·미지원 플래그/값 오류 |
| `lane *`     | 성공                             | 값 검증 실패(예: 미지원 필드 값, 정체성 필드 편집 거부)·미지원 하위명령                            | 필수 인자 누락·미지원 플래그/값 오류      |
| `proj *`     | 성공                             | `--force` 없는 삭제 가드 발동·미지원 하위명령·오류                                                 | 필수 인자 누락·미지원 플래그/값 오류      |
| `completion` | 스크립트 출력                    | 미지원 셸                                                                                          | 셸 인자 누락                              |
| `init`       | 마법사 완료                      | 비TTY·인자 누락·검증/생성 오류                                                                     | — (이 종료 코드 체계 밖 — 아래 참조)      |
| `alias`      | 별칭 설치·이미 설정 확인         | `adde` PATH 미발견·설치 실패                                                                       | —                                         |

`init`/`alias` 는 이 체계 이전부터 있던 명령이라 편입되지 않았습니다 — 여전히 `0`/`1` 만 씁니다(인자 누락도 `2` 가 아니라 `1`).

인자 없이 실행하거나 `-h`/`--help`/`help` 는 사용법을 출력하고 `0` 을 반환합니다. **미지원 명령**(오타 등)과 **미지원 하위명령**(예: `adde lane foo`)은 stderr 에 `Unknown command` 또는 그에 준하는 메시지를 출력하고 `1` 을 반환합니다(`2` 가 아님) — 식별된 (하위)명령에 대한 잘못된 호출과는 구분되는 별개 사례입니다(스크립트에서 오타가 조용히 성공으로 처리되는 것을 막음).
