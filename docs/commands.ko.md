...
adde lane show <proj> <lane> [--json]            # conf 출력
...
**`--json`**: `lane show --json` 은 사람용 덤프 대신 `{ "v": 1, "lane", "confPath", "conf" }`(`conf` 는 파싱된 레인 conf 객체 — 토큰 필드 없음)를 출력합니다. `v` 는 스키마 버전입니다(전역 옵션 참조).
