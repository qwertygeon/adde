...
| `--add-allow <a,b,c>` `--rm-allow <a,b,c>`                               | **add to / remove from** the allowlist (merged with the current list, not replaced) — see below                 |
| `--add-deny <entries,...>` `--rm-deny <entries,...>`                     | add to / remove from the denylist                                                                               |
| `--add-hard-deny <entries,...>` `--rm-hard-deny <entries,...>`           | add to / remove from the hard-deny list                                                                         |
...
**Positional dot-notation keys**: `adde lane set <proj> <lane> <key> <value> …` edits by canonical dot-notation key (e.g. `perm_tier autopass`, `markdown.retention_days 5`), taking one or more `<key> <value>` pairs in a single command. This is the same surface as the named flags above, **plus these markdown-only keys that have no flag**: `markdown.archive`, `markdown.backup`, `markdown.retention_days`, `markdown.out_retention_days`, `markdown.sync_provider`, `markdown.layout`, `markdown.palette`, `markdown.records_cap` (see the [markdown guide](markdown.md#1-lane-configuration) for what each does). A batch is **all-or-nothing**: an unknown key (with a "did you mean…" suggestion for near-misses), a wrong type/enum/format value, or an odd number of tokens rejects the whole command and writes nothing.

**Incremental list editing** (`--add-allow`/`--rm-allow`, `--add-deny`/`--rm-deny`, `--add-hard-deny`/`--rm-hard-deny`): edit one entry of a security list without retyping the whole thing. Each takes a comma-separated set that is merged into (`--add-*`) or subtracted from (`--rm-*`) the lane's current list. Removing an entry that isn't present is a no-op (a notice lists what was ignored). You cannot combine a whole-list replacement (`--allowlist`) with its incremental variants in the same command, and passing the same entry to both add and remove is rejected. On an `autopass` lane whose denylist is still the built-in default, an incremental denylist edit keeps the default entries (they are seeded first, then your add/remove is applied) so a single `--add-deny`/`--rm-deny` never silently drops the default protections.
