...
- **세션**: 대화 1건. 자체 식별자·엔진·수명 상태를 가짐:

  | 상태         | 감시      | 시점                                                                                                                                        |
  | ------------ | --------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
  | `active`     | 함        | 엔진 상주, 턴 수신 준비됨                                                                                                                   |
  | `hibernated` | 함        | 엔진 비상주, 다음 턴에서 투명하게 재개(기본 30분 유휴 후)                                                                                   |
  | `stopped`    | **안 함** | 직접 중지했거나 `session clear` 로 승계됐거나 1시간 무활동으로 자동 중지된 상태. 재개할 때까지 입력 노트·승인 디렉터리를 아예 읽지 않습니다 |
  | `detached`   | **안 함** | 재개가 실패했거나 엔진 자가 재기동을 포기한 상태 — 사유가 기록되고 재개 목록에 함께 표시됩니다                                              |

  신규 세션의 식별자는 `260828-2` 같은 형태(`YYMMDD-N`, 제목이 있으면 `YYMMDD-N-<slug>`)여서 목록에서 눈으로 알아보고 지목할 수 있습니다. 이전에 부여된 식별자는 구 형식을 그대로 유지합니다.
...
두 경로는 모두 **절대경로**여야 하며, 앞의 `~/` 는 홈 디렉터리로 확장됩니다. 상대경로는 설정을 읽는 시점에 거부되므로 값을 고칠 때까지 그 프로젝트의 명령과 데몬이 동작하지 않습니다 — [명령 레퍼런스](commands.ko.md#project--프로젝트-관리) 참조.
...
```bash
adde session ls myproj             # 세션 목록(최근 활동 순)
adde session show myproj <sid>     # 세션 상세
adde session clear myproj <sid>    # 승계 — 현재 세션 중지 + 새 세션 생성(삭제 아님)
adde session stop myproj <sid>     # 당분간 안 쓸 세션 — 감시에서 빠집니다
adde session resume myproj <sid>   # 중지된 세션 되살리기
```

중지된 세션은 사라진 것이 아닙니다 — 노트와 전체 이력이 vault 에 그대로 남고, 입력 노트만 "더 이상 감시되지 않으며 어떻게 되살리는지" 를 담은 짧은 안내형으로 1회 교체됩니다. 방치한 세션은 무활동 1시간 후 자동으로 중지되며, `adde project set <proj> idle_stop false` 로 끄고 `stop_after_min` 으로 임계를 바꿉니다.
...
**성공 확인**: `adde status <proj>` 에서 세션이 `active`(또는 유휴 후 `hibernated` — 정상)로 보이면 데몬과 그 세션은 정상입니다. `stopped` 도 정상적인 휴지 상태입니다 — 재개할 때까지 그 세션을 아무도 보고 있지 않다는 뜻일 뿐입니다. `detached` 는 조치가 필요하다는 뜻입니다 — [트러블슈팅](troubleshooting.ko.md) 참조.
...
모든 프로젝트와 세션을 — 설정은 물론 ADDE 가 vault 에 쓴 노트까지 — 지우려면 제거 **전에** `adde factory-reset` 을 실행하세요. 인벤토리를 먼저 보여 주고 고정 문구 타이핑을 요구하며, 터미널 밖에서는 실행을 거부하고, vault 루트 자체와 v2 이전 데이터는 건드리지 않습니다. 읽을 수 없는 것은 지우지 않고 열거합니다 — 설정 파일이 손상된 프로젝트, vault 경로를 해석할 수 없는 프로젝트(후자는 "설정만 지울지" 를 별도로 묻습니다). [명령 레퍼런스](commands.ko.md#factory-reset--전체-초기화) 참조.
