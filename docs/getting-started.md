...
ADDE 는 AI CLI 엔진(Claude Code 등)을 채널(Telegram / Obsidian)에서 원격 구동하는 게이트웨이입니다. 이 문서는 설치부터 첫 레인 기동까지를 다룹니다.
...
- **소스(source)**: 지시를 받는 채널. `telegram`(봇 long-poll) 또는 `obsidian`(노트 파일 감시).
...
```ini
source=telegram         # telegram | obsidian
backend=acp
...
- **obsidian**: `vault=<절대경로>`, `inbox=<vault 상대>`, (선택) `approvals=`·`outbox=`. → [Obsidian 가이드](obsidian.md).
...
- Obsidian 으로 메모 기반 구동: [obsidian.md](obsidian.md)
