...
- ⚠️ **autopass 는 옵트인 자동 허용 모드**: `perm_tier=autopass` 면 denylist 에 없는 모든 도구가 자동 허용되고, denylist 도구만 승인 노트가 생성됩니다(전량 트랜스크립트 기록). 기동·운영 경고는 outbox 의 `_adde-notice.md` 노트로 도착합니다. 티어·denylist 선택 기준은 [권한 가이드](permissions.md).
...
처리되면 해당 줄이 `✅ sent [[...]]` 로 종단되고 결과(완료 통지·세션 목록)가 응답 노트로 링크됩니다. 제어 라벨은 send 처럼 메시지 경계로 작동하므로 **별도 줄에 단독으로** 두세요. 상세: [명령 레퍼런스](commands.md#세션-제어-채널-명령).
...
`allowlist` 에 도구를 넣어두면 그 도구는 매번 묻지 않아 승인 빈도가 줄어듭니다(게이트 자체는 유지). 권한 모델 전반은 [권한 가이드](permissions.md).
...
증상별 진단·조치는 [트러블슈팅](troubleshooting.md#마크다운-전용)을 참고하세요 (마크다운 전용 표 포함). 권한이 항상 거부되면 10분 타임아웃 전에 allow/deny 중 정확히 하나만 체크했는지 확인하세요.
