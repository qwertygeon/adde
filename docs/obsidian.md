# Obsidian 으로 ADDE 사용하기

버튼이 없는 Obsidian 에서는 **노트 파일 편집만으로** AI 레인을 구동합니다. 지시는 인박스 노트에 체크박스로 보내고, 권한은 승인 노트의 체크박스로 허용/거부하며, 응답은 출력 노트로 받습니다. 모바일(Obsidian Sync / Syncthing 동기)에서도 동일하게 동작합니다.
...
```
[Obsidian inbox 노트]  --(send 체크박스)-->  ADDE  --(ACP)-->  AI 엔진(claude 등)
[Obsidian approvals 노트] <--(권한 요청 블록)-- ADDE
[Obsidian approvals 노트]  --(allow/deny 체크)-->  ADDE (게이트 반영)
[Obsidian out/ 노트]  <--(응답 1메시지=1파일)-- ADDE
```

- 한 레인 = `(Obsidian 노트 ↔ 프로젝트 폴더)` 한 쌍. 여러 레인을 등록해 여러 메모·폴더를 각각 매핑할 수 있습니다.
...
vault=/Users/me/ObsidianVault

# vault 상대 경로 — 입력 노트(필수)
...
- `vault` 만 절대경로, `inbox`·`approvals`·`outbox` 는 vault 기준 상대경로입니다.
- 입력 노트(`inbox.md`)는 Obsidian 에서 직접 만들어 두세요(없으면 지시를 받을 수 없습니다).
...
2. 그 **아래 줄**에 send 체크박스를 만듭니다:
   ...
   - [ ] 📤 send
   ```
3. 보낼 준비가 되면 체크박스를 탭해 체크합니다: `- [x] 📤 send`.
4. ADDE 가 감지해 메시지를 AI 에 전달하고, 그 줄을 **종단 마커**로 바꿉니다:
   ```markdown
   - [x] ✅ sent a1b2c3d4
   ```
   이 줄이 보이면 전송 완료입니다.
...
> 체크박스 문구는 단어 **send** 만 있으면 됩니다(이모지는 장식). 빈 메시지를 체크하면 전송되지 않고 `⚠️ empty` 로 표시됩니다.
...
`lanes.d/` 에 conf 파일을 여러 개 두면 레인이 여러 개 동시에 구동됩니다. 각 레인은 자기 `vault`/`inbox`/`approvals`/`outbox` 와 `cwd`(프로젝트 폴더)를 가지므로, **메모와 프로젝트 폴더를 각각 매핑해 N개 등록**할 수 있습니다.
...
| 레인이 안 뜸 | `vault` 절대경로가 실제 존재하는지(없으면 fail-closed 로 기동 거부) |
