...
- **셸 체이닝**: Bash 명령은 체이닝·그룹의 하위 명령을 개별 매칭합니다(`;` `&&` `||` `|` `&`·그룹 `(` `)` `{` `}`·`$(…)`·백틱·개행으로 분리, 선행 `VAR=` 대입 제거) — 접두 패턴(`sudo *`)이 `echo x && sudo y`·`(sudo y)`·`FOO=1 sudo y` 를 잡습니다. 완전한 셸 파서가 아닌 best-effort 입니다: alias·`eval`·변수 확장·래퍼 호출(`bash -c "sudo y"` 는 **못 잡음**)은 해석하지 않고, 따옴표 안 연산자에서도 분리하므로 — `--safe-defaults` 에서는 정상 명령이라도 인용부 인자에 연산자+위험 토큰이 들어있으면(예: `git commit -m "fix && sudo cleanup"`) 우회로 없이 거부될 수 있습니다. 확실한 차단은 도구 전체(`Bash`)를 지정하세요.
- **게이트는 엔진이 올려주는 요청만 봅니다.** ADDE 는 엔진이 ACP `requestPermission` 으로 승인을 요청할 때만 그 도구를 평가합니다. 하부 claude 엔진은 자체 설정(`~/.claude` allowlist / permission mode)에 따라 ADDE 에 묻기 전에 도구를 **자동 허용**할 수 있고, 그러면 ADDE 게이트(`hard_deny` 포함)는 관여하지 않습니다. 어떤 도구가 `denylist`/`hard_deny` 에 걸리지 않고 실행된다면, claude 자체 권한 설정(`~/.claude` 의 allowlist / permission mode)을 직접 조정하세요. (아래 _권한 드리프트 경고_ 참조.)
