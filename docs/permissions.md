...
- **The gate only sees what the engine escalates.** ADDE evaluates a tool only when the engine asks for approval via ACP `requestPermission`. The underlying claude engine may **auto-approve a tool per its own settings** (`~/.claude` allowlist / permission mode) before ever asking ADDE — in which case ADDE's gate (including `hard_deny`) is not consulted. If a tool runs without being caught by your `denylist`/`hard_deny`, adjust the engine's own permission settings in `~/.claude` (allowlist / permission mode) directly. (See also _Permission-drift warning_ below.)
