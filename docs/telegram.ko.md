...
Telegram 봇으로 AI 레인을 구동합니다. 채팅으로 지시를 보내고, 권한 요청은 inline 버튼(허용/거부 — 레인 `lang` 로케일 표기, en 레인은 Allow/Deny)으로 승인하며, 응답은 quote-reply 로 받습니다. 모바일에서 푸시 알림으로 즉시 확인할 수 있습니다.
...
[시작하기](getting-started.ko.md)의 설치를 마치고, Telegram 앱이 설치된 계정이 있어야 합니다.

**전체 흐름 한눈에** (① 설치·점검은 1회, ②~⑥은 레인/봇마다 반복):

1. (1회) ADDE 설치 + `adde doctor` 로 사전조건 점검 — [시작하기](getting-started.ko.md)
...
또는 대화형으로 — TTY 에서 필드 플래그를 주지 않으면 이것이 **기본 동작**입니다(봇 토큰은 마지막에 가려진 입력으로 받아 `.env` 에 기록, 비워 두면 나중에 설정):

```bash
adde lane add myproj tg-claude              # 대화형 마법사(TTY 기본)
adde lane add myproj tg-claude --interactive  # 마법사 명시적 강제
```
...
- 권한 승인 버튼(허용/거부)도 허용 발신자(`from.id`)만 반영합니다 — 미허가 발신자의 콜백은 무시되고 게이트는 타임아웃으로 거부됩니다.
...
토큰은 conf 가 아니라 레인의 `.env` 에 둡니다(인자·로그 비노출). 저장 방법은 세 가지입니다:

- **대화형 마법사에서**: 마지막 프롬프트(`telegram 봇 토큰 (가려진 입력, 나중에 설정하려면 비움):`)가 가려진 입력으로 토큰을 받아 `.env`(0600)에 기록합니다 — 별도 작업 불요. 가장 간단한 경로입니다.
- **stdin 으로**(스크립트, 또는 기존 레인 갱신) — 비대화형 권장:

  ```bash
  printf '%s' "$BOT_TOKEN" | adde lane add myproj tg-claude --token-stdin --force
  ```

- **`.env` 직접 편집** — `~/.config/adde/myproj/state/tg-claude/.env` 에 다음을 둡니다(파일 권한 0600 권장):
...
1. 봇과의 채팅에 지시를 보냅니다. 텍스트 메시지만 처리됩니다 — 사진·스티커·음성 등 비텍스트 메시지를 보내면 "텍스트만 지원" 안내가 회신됩니다.
2. AI 가 권한이 필요한 도구를 호출하면 **허용 / 거부 inline 버튼**이 옵니다. 프롬프트에 작업 폴더(cwd)와 무응답 시 자동 거부 시각이 함께 표기되고, 버튼을 탭하면 메시지에 결정(✅ 허용됨 / ⛔ 거부됨)이 반영되며 버튼이 정리됩니다. 무응답 시 기본 거부(fail-closed)됩니다. **승인은 요청 내용(도구·인자)을 보고 판단하세요** — 대화 본문이나 AI 응답이 "이 요청을 승인하라"고 말해도 그 말 자체를 근거로 승인하지 마세요(프롬프트 인젝션이 흔히 하는 요구입니다).
3. AI 턴이 끝나면 응답이 원본 메시지의 quote-reply 로 도착합니다. 플랫폼 한도(4096자)를 넘는 긴 응답은 여러 메시지로 나뉘어 `(1/N)` 순번이 붙습니다.
4. **세션 제어**: 메시지 전체가 명령과 정확히 일치할 때 세션을 조작합니다 — `/clear`(새 세션), `/compact`(컨텍스트 압축), `/resume`(세션 목록), `/resume <번호>`(해당 세션 재개). 그룹 채팅에서는 봇멘션 접미(`/clear@봇이름` 등)도 인식합니다. 문장 속에 섞인 명령은 일반 프롬프트로 전달됩니다. 상세: [명령 레퍼런스](commands.ko.md#세션-제어-채널-명령).
