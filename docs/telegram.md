...
4. **세션 제어**: 메시지 전체가 명령과 정확히 일치할 때 세션을 조작합니다 — `/clear`(새 세션), `/compact`(컨텍스트 압축), `/resume`(세션 목록), `/resume <번호>`(해당 세션 재개). 문장 속에 섞인 명령은 일반 프롬프트로 전달됩니다. 상세: [명령 레퍼런스](commands.md#세션-제어-채널-명령).
