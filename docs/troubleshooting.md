...
`npm i -g adde` 직후 레인 기동 전에 마주치는 문제들입니다.
...
| 설치 시 `EACCES` 권한 오류              | root 소유 Node prefix                | `sudo` 대신 버전 매니저(nvm/fnm) 또는 사용자 prefix(`npm config set prefix ~/.local`) 사용 — [시작하기 설치 절](getting-started.md#설치)            |
