...
| `adde down <proj>` right after `adde up <proj>` ends abnormally instead of cleanly                 | Known limitation: a stop signal delivered within roughly a millisecond of process start — before the daemon has run any of its own code — is handled by the OS default (immediate termination), not `adde`'s graceful shutdown | Wait briefly after `up` before sending `down`; `adde status <proj>` and `adde up <proj>` recover normally afterward                                                                                                                                                                                                    |
