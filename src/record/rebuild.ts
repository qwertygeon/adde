 ...
 * 재구성. 보관 정책을 필수 입력으로 받아 이미 이관된 턴 노트를 vault 에 재생성하지 않는다(ADR-023b).
 *
 * **계약 편차 고지(GAP-011)**: tasks.md 확정 시그니처는 `rebuild(proj, opts)` 이나, 세션 열거·경로
...
/** dedup 원장 재구성 — 세션 간 시각순으로 재생해 최초/중복 매핑을 결정론적으로 재도출한다. */
async function rebuildDedup(
  ...
  for (const sid of sids) {
    for await (const e of readEvents({ base, vaultRoot, proj, sid })) {
      if (e.t === "turn_start") {
        tagged.push({ kind: "user_input", text: e.input.text, sid, turn: e.turn, ts: e.ts });
      } else if (e.t === "text_final" && typeof e.text === "string") {
        tagged.push({ kind: "assistant", text: e.text, sid, turn: e.turn, ts: e.ts });
      }
    }
  }
  ...
  for (const item of tagged) {
    ...
    lines.push(
      JSON.stringify({
        ...
        hash,
        kind: item.kind,
        first: { sid: seen.sid, turn: seen.turn },
        dup: { sid: item.sid, turn: item.turn },
        ts: item.ts,
      }),
    );
  }
 ...
 * 바이트 동일해야 한다(SC-016·SC-047) — 모든 쓰기는 `projectTurn`/`project`(atomicWrite) 경유.
 */
  ...
  for (const sid of sids) {
    ...
    for (const turn of turns) {
      if (opts?.retention && isArchivedTurn(opts.retention, await turnStartIsoOf(ctx, turn))) {
        continue; // 이관된 턴은 재생성하지 않는다(ADR-023b) — 세션 노트가 "보관됨" 으로 표기.
      }
