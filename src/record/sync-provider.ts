 ...
 * 현행 `src-adapters/sync-provider.ts` **그대로 이식**(동작 무변경, design.md ADR-023). 신규
 * 제공자는 레지스트리 등록만으로 추가되며 `runRetention` 호출부는 코드 변경이 필요 없다.
 */
 ...
 * 로컬에 없는 placeholder 로 간주한다. 감지 실패(예외·플랫폼 미지원)는 보수적으로 "이미 로컬"
 * (false)로 간주해 불필요한 대기를 만들지 않는다.
 */
