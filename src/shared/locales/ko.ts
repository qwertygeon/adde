...
명령별 도움말은 \`{{primary}} <command> --help\`, 레인 옵션은 \`adde lane help\` 참조.`,
  ...
  --json       기계가독 출력({proj, stopped: true, wasRegistered})`,
...
전역 설치에서만 동작(PATH 의 adde 옆 쓰기 가능한 bin 디렉터리 필요)하며, 동명 명령이 이미 있으면 덮어쓰지 않고 건너뜁니다.`,
  ...
  adde lane show <proj> <lane> [key] [--json] [--defaults] 레인 conf 출력 (단일 key + --json 은 value/default/explicit/editable/identity 표시)
...
참고: --file-mode 편집은 conf 값만 갱신하며, 재시작 후에도 기존 디렉터리 권한은 변경되지 않습니다(private→shared 완화는 수동 chmod 필요). file_mode 는 내부 state/out/queue 디렉터리만 지배하며 마크다운 노트 트리는 대상이 아닙니다.`,
  ...
  cli: {
    ...
    unknownProjSub: "알 수 없는 proj 서브커맨드: {{sub}}",
  ...
  run: {
    ...
    downNotRunning: "[adde] {{proj}}: 등록된 데몬 없음 — 정지할 대상이 없습니다.",
  ...
  ops: {
    ...
    sessions: {
      hint: "재개는 채널에서 합니다: /resume <n> 전송(또는 markdown 레인의 resume 체크박스 체크).",
    },
    logs: {
      ...
      daemonNotFound:
        "데몬 로그 없음: {{path}}\n  ↳ 조치: {{proj}} 데몬이 아직 실행되지 않았거나 아무것도 기록하지 않았습니다. adde up {{proj}} 로 기동하세요.",
      empty: "({{path}} 비어있음)",
    },
  },
  lane: {
    retry: {
      ...
      root: "root — 필수 (markdown 루트 절대경로를 입력하세요)",
    },
    prompt: {
      source: "source (markdown = 노트 파일로 구동 / telegram = 봇 채팅으로 구동)",
      enumHint: "번호 또는 값 입력",
      enumInvalid: "잘못된 입력 — 위 번호 중 하나 또는 값을 그대로 입력하세요:",
      permTier: "perm_tier (acp = 도구마다 채널 승인 / autopass = denylist 외 자동 허용)",
      allowlist: "allowlist (콤마 구분, 없으면 비움)",
      denylist: "denylist (채널 승인으로 폴백할 도구·패턴, 콤마 구분; 없으면 권장 기본 목록)",
      safeDefaults:
        "방어심화 하드-거부 기본값을 켤까요? sudo / rm -rf / git 강제 / 자격증명 읽기를 즉시 차단",
      ...
      root: "root (markdown 루트 절대경로, 필수)",
      inbox: "inbox (root 상대)",
      approvals: "approvals (root 상대, 없으면 기본 `approvals/`)",
      outbox: "outbox (root 상대, 없으면 기본 `out/`)",
      hardDeny: "hard_deny (티어 무관 즉시 거부할 도구·패턴, 콤마 구분)",
      archive: "markdown.archive (root 상대; 전송 본문 이관 위치, 없으면 비움)",
      backup: "markdown.backup (로컬 백업 폴더 경로, 없으면 비움)",
      retentionDays: "markdown.retention_days (이관 기준일, 캘린더일, 없으면 기본 2)",
      outRetentionDays:
    ...
    set: {
      ...
      noChange: "변경 없음 — 갱신할 내용이 없습니다.",
      wizardHeader:
        '레인 "{{lane}}" 편집 — 비워 두면 현재값(괄호 안)을 유지합니다. 기본값 복원은 adde lane set ... --unset <key> 를 쓰세요.',
      diffHeader: "적용할 변경:",
      diffLine: "  {{key}}: {{from}} → {{to}}",
      confirm: "이 변경을 적용할까요?",
      aborted: "취소됨 — 변경 사항 없음.",
    },
    show: {
      line: "{{key}}: value={{value}} default={{default}} explicit={{explicit}} editable={{editable}} identity={{identity}}",
      defaultsHeader: "편집 가능 키 (key = 기본값):",
      unset: "(미설정)",
    },
    noLanes: "{{proj}}: 레인 없음",
  ...
  init: {
    ...
    aliasPrompt: "짧은 별칭({{names}})을 adde 명령 옆에 설치할까요?",
    completionPrompt: "{{shell}} 셸 탭 자동완성을 지금 설정할까요? (실행할 명령을 출력)",
  ...
  laneConfig: {
    ...
    err: {
      ...
      unsetNoKeys:
        "--unset 은 키가 하나 이상 필요합니다 — 사용법: adde lane set <proj> <lane> --unset <key> ...",
      denylistNoopAcp:
      ...
      unknownKeyDidYouMean:
        'key "{{key}}" 는 편집 가능한 레인 키가 아닙니다 — 혹시: {{suggestions}}?',
      badIntValue: '{{key}} 는 양의 정수여야 합니다 — 입력값 "{{value}}"',
      badEnumValue: '{{key}} "{{value}}" 가 올바르지 않습니다 — {{allowed}} 중 하나',
      requiredUnset: 'key "{{key}}" 는 필수라 unset 할 수 없습니다(변경하려면 레인을 재생성하세요)',
      keyValueIncomplete:
        "각 key 에는 value 가 필요합니다 — 사용법: adde lane set <proj> <lane> <key> <value> ...",
    },
  },
  telegram: {
    permPrompt: "권한 요청: {{tool}}\n{{detail}}",
    permPromptCwd: "📁 작업 폴더: {{cwd}}",
    permPromptDeadline: "🕒 무응답 시 {{deadline}} 자동 거부",
    permAllowed: "✅ 허용됨",
    permDenied: "⛔ 거부됨",
    permBtnAllow: "허용",
    permBtnDeny: "거부",
    nonTextUnsupported: "⚠️ 텍스트 메시지만 지원합니다. 요청을 텍스트로 보내주세요.",
    enqueueFail: {
  ...
  markdown: {
    ...
    outMeta: "🕒 전송 {{sent}} · 완료 {{done}}",
    approvalMeta: "🕒 요청 {{requested}} · 무응답 시 {{deadline}} 자동 거부",
    approvalHint: "아래에서 정확히 하나만 체크하세요 — allow 또는 deny (둘 다 비우면 대기 유지)",
    backupPathOverlap:
  ...
  injector: {
    ...
    control: {
      ...
      sessionsItem: "{{n}}. {{label}} — 마지막 활동 {{last}} ({{id}})",
  ...
  acp: {
    spawnFail: {
      ...
      action: "어댑터 바이너리 설치(pnpm install)를 확인한 뒤 adde up 을 재시도하세요.",
    },
    handshakeTimeout: {
      situation: "엔진 핸드셰이크({{phase}}) {{seconds}}초 내 무응답",
      action: "엔진 바이너리·헬스를 확인한 뒤 adde up 을 재시도하세요.",
    },
  ...
  log: {
    ...
    telegram: {
      ...
      nonTextReplyError: "[telegram] 비텍스트 회신 실패: {{error}}",
