...
/** 재검증 폴 간격(ms). */
const ICLOUD_POLL_INTERVAL_MS = 500;

/**
 * macOS iCloud dataless(placeholder) 감지 — 실기기 실측으로 확정하기 전의 잠정 휴리스틱.
 * `stat` 의 `blocks`(할당된 512바이트 블록 수)가 0인데 `size`(논리 크기)가 0보다 크면 콘텐츠가
 * 로컬에 없는 placeholder 로 간주한다(일반 파일은 콘텐츠만큼 블록을 점유). 감지 실패(예외·플랫폼
 * 미지원)는 보수적으로 "이미 로컬"(false)로 간주해 불필요한 대기를 만들지 않는다 — 오탐으로
 * dataless 를 놓쳐도 safeMove 의 copy-후-크기검증(fs-move.ts)이 최종 방어선이 된다.
 */
...
const icloudProvider: SyncProviderDescriptor = {
  ...
  async ensureLocal(path: string): Promise<SyncMaterialize> {
    ...
    // 다운로드 트리거·구체 API(brctl/NSURL 등)는 실기기 실측으로 확정할 사항 — 현재는 재검증
    // 폴링으로 다운로드 완료(OS 가 read/stat 접근 자체로 다운로드를 유발)를 유계 대기한다.
    const deadline = Date.now() + ICLOUD_DOWNLOAD_TIMEOUT_MS;
    while (Date.now() < deadline) {
      if (!(await isDataless(path))) return "ready";
      await new Promise((r) => setTimeout(r, ICLOUD_POLL_INTERVAL_MS));
    }
