...
describe("watch 오류 시 통지 후 폴링 지속 — 조용한 멈춤·크래시 없음", () => {
  it("watch 가 오류로 끊기면 onWatchError 가 1회 통지되고, 안전망 폴링이 append 를 계속 방출한다", async () => {
    const { followFile } = await import("../../src/core/log-follow.js");
    const chunks: string[] = [];
    ...
    let size = 0;
    const stat = vi.fn(async () => ({ ino: 1, size }));
    const content = "after-watch-error\n";
    const read = vi.fn(async (_p: string, offset: number, length: number) =>
      Buffer.from(content, "utf8").subarray(offset, offset + length),
    );
    // onError 를 캡처해 두었다가 감시 오류 발생을 모사하는 fake(오류 후 이벤트 발화 없음).
    let fireError: ((err: unknown) => void) | undefined;
    const watch = vi.fn(
      (
        _dir: string,
        _listener: (e: string, f: string | null) => void,
    ...
    const done = followFile("dummy-target", {
      ...
      signal: ac.signal,
      pollMs: 10,
      onWatchError: (err) => watchErrors.push(err),
      deps: { stat, read, watch },
      startOffset: 0,
      startIno: 1,
    });

    await waitFor(() => fireError !== undefined, { timeoutMs: 1000 });
    fireError!(Object.assign(new Error("EPERM"), { code: "EPERM" })); // 감시 오류 발생
    size = content.length; // 오류 이후 append — 폴링만으로 감지되어야 한다

    await waitFor(() => chunks.join("").includes("after-watch-error"), { timeoutMs: 1000 });
    ac.abort();
    await done;

    expect(watchErrors).toHaveLength(1); // 통지 정확히 1회(무로그 흡수 아님)
    expect(chunks.join("")).toContain("after-watch-error"); // 추적 지속(조용한 멈춤 없음)
  });
});
